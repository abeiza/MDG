...
<script>
	$(function(){
		...
		$("#administrator-tab").click(function(){
			$("#all-tab").css('background-color','#fff');
			$("#administrator-tab").css('background-color','#f1f1f1');
			$("#regular-tab").css('background-color','#fff');
			$(".tr-Uploader").hide();
      $(".tr-Administrator").show();
		});

		$("#regular-tab").click(function(){
			$("#all-tab").css('background-color','#fff');
			$("#administrator-tab").css('background-color','#fff');
			$("#regular-tab").css('background-color','#f1f1f1');
			$(".tr-Administrator").hide();
      $(".tr-Uploader").show();
		});
...
<script>
	$(function(){
		$(function(){
			$.ajax({
				url:"<?php echo base_url();?>index.php/control/get_list_users_all/",
				cache:false,
				type: "POST",
				dataType: 'json',
            ...
            $("#tbl-all tbody").append("<tr class='tr-"+data.Level_Name+"' style='font-size:12px' id='"+data.Account_ID+"' onclick='get_edit(this)' style='Font-size:13px;font-family:Roboto-Regular'><td>"+data.Account_First_Name+" "+data.Account_Last_Name+"</td><td>"+data.Account_Email+" - "+data.Account_Phone+"</td><td>"+data.Level_Name+"</td><td>"+data.Position_Name+"</td><td>"+data.Department_Name+"</td><td>"+stts+"</td></tr>");
					});
				}
			}
    );
...
<script>
	...
	function get_add(){
		window.location.href = '<?php echo base_url()?>index.php/control/create_users/';
	}

	function get_edit(el){
		window.location.href = '<?php echo base_url()?>index.php/control/update_users/'+el.id;
	}
  ...
  function linkDisplayInbox(id){
    window.location.href = '<?php echo base_url();?>index.php/control/get_detail_confirm/'+id.id;
  }
	...
	<div class="container">
		<div class="bg-list">
			...
			<div class="middle-box">
				...
				<div class="group-btn">
					<button id="add-btn" onclick="get_add();" style="color:#fff;font-size:12px;float:left;margin-right: 5px;;">Add</button>
					<button id="print-btn" onclick="get_print();"></button>
					<button id="refresh-btn" onclick="listDetail();"></button>
				</div>
				<div style="float: right;margin-right: 65px;"><h3>Confirm List</h3></div>
			...
			<div id="tab-table" style="float:left;width:100%;margin-top: 20px;">
				...
				<a class="tab" id="administrator-tab">Master Administrator</a>
				<a class="tab" id="regular-tab">Master Regular</a>
			</div>
