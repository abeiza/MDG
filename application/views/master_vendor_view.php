...
<script>
	$(function(){
		$(function(){
			$.ajax({
				url:"<?php echo base_url();?>index.php/request/get_list_ms_vendor/",
				cache:false,
				type: "POST",
				dataType: 'json',
            ...
            if(data.MDG_Status == 2){
              ...
              status = '';
						...
						//$("#tbl-all tbody").append("<tr><td>Request ID (Vendor)</td><td>Vendor Name</td><td>Vendor Type</td><td>Posted Date</td><td>Created By</td></tr>");
						$("#tbl-list tbody").append("<tr class='tr-vendor' style='font-size:12px' id='"+data.MDG_Vendor_ID+"' onclick='linkDisplayInbox(this)' style='Font-size:13px;font-family:Roboto-Regular'><td>"+data.MDG_VendorType_ID+" - "+data.VendorType_Name+"</td><td>"+title+" "+data.MDG_VendorName+"</td><td>"+data.MDG_City+"</td><td>"+data.MDG_Country+"</td><td>"+status+"</td></tr>");
					});
				}
			});
		});
...
<script>
  ...
  function linkDisplayInbox(id){
    $(function(){
			var iden = '';
      $.ajax({
        url:"<?php echo base_url();?>index.php/request/get_data_detail_draft/"+id.id,
        cache:false,
        type: "POST",
        dataType: 'json',
        success:function(result){
		...
		<div class="bg-list">
			...
			<div class="list-box">
  			<div id="tab-table" style="float:left;width:100%;margin-top: 20px;">
  				<a class="tab" id="list-tab">List Vendor</a>
