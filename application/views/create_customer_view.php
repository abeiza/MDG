			...
			$(function(){
				$.ajax({
					...
					success:function(result){
						...
						$.each(result, function(i, data){
								$("#customer_city_desktop").append("<option value='"+data.ObjectID+"'>"+data.ObjectID+" - "+data.CustomerCity_Name+"</option>");
						});
					}
				});
			});
			...
			$(function(){
				$.ajax({
					...
					success:function(result){
						...
						$.each(result, function(i, data){
								$("#customer_paymentterm_desktop").append("<option value='"+data.ObjectID+"'>"+data.ObjectID+" - "+data.MDG_TermName+"</option>");
						});
					}
				});
			});
	...
	<script>
		...
		function get_send(){
			...
			$load_id.show();
			$.ajax({
				url:"<?php echo base_url();?>index.php/request/get_send_customer/",
				cache:false,
				data:{
				...
				success:function(result){
					if(result.status == 'success'){
						...
						$(".span-message span").remove();
						$(".span-message").append('<span>ID Customer not to use, Please Refesh this form and try again . .</span>');
						$("#noticeCnt").fadeIn().delay(3000).fadeOut();
		...
		function get_save(){
			...
			$load_id.show();
			$.ajax({
				url:"<?php echo base_url();?>index.php/request/get_save_customer/",
				cache:false,
				data:{
					id:$('#customer_id_desktop').val(),
					title:$('#customer_title_desktop').val(),
			...
			<div class="bg-list">
				...
				<div class="list-box">
					<div class="form-create">
						<h3 style="margin-top:20px;">Request Master Data Customer</h3>
						<div style="margin-top:20px;margin-bottom:50px;">
							<div id="frmDTL">
