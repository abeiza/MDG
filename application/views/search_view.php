...
<script>
	$(function(){
		$(function(){
			$.ajax({
				url:"<?php echo base_url();?>index.php/request/get_list_search_all/",
        data:{
          src:'<?php echo urldecode($this->uri->segment(3));?>'
        },
				cache:false,
				type: "POST",
				dataType: 'json',
            ...
            $("#tbl-all tbody").append("<tr class='tr-"+data.Code+"' style='font-size:12px' id='"+data.MDG_ID+"' onclick='linkDisplayInbox(this)' style='Font-size:13px;font-family:Roboto-Regular'><td>"+data.Code+"</td><td>"+data.type_id+" - "+data.typename+"</td><td>"+data.name+"</td><td>"+data.address+"</td><td>"+data.name_province+"</td><td>"+data.first+" "+data.last+"</td></tr>");
					});
				}
			}
    );
...
<script>
  ...
  function linkDisplayInbox(id){
    window.location.href = '<?php echo base_url();?>index.php/request/get_detail_search/'+id.id;
  }
	...
	<div class="container">
		<div class="bg-list">
			...
			<div class="middle-box">
				...
				<div style="float: right;margin-right: 65px;"><h3>Search Result for keywords : <?php echo urldecode($this->uri->segment(3));?></h3></div>
